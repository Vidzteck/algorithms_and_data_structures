...
\[T(n) = O(n) + O(n-1) + O(n-2) + ... + O(1) = O(n(n+1)/2) = O(n^2)\]
...
**Вопрос 7: Какова временная сложность задачи о N ферзях?**

## Ответ:
Временная сложность наивного алгоритма решения задачи о N ферзях с использованием метода backtracking составляет O(N!).

## Обоснование:
* На первой строке можно разместить ферзя N способами.
* На второй — в худшем случае (N−1) способов (т.к. один столбец занят и диагонали могут блокировать ещё позиции).
* Продолжая, получаем приблизительно N × (N−1) × (N−2) × … × 1 = N! возможных расстановок, которые алгоритм может перебрать.
* Хотя часть ветвей отсекается проверкой isSafe, асимптотически сложность остаётся O(N!).
