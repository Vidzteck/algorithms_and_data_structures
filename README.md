...
Временная сложность реализованного алгоритма составляет **O(E²)**, где `E` — количество ребер в графе. (Примечание: `n` часто обозначает количество вершин `V`, но в данном случае сложность выражена через количество ребер `E`).

---

**3) Почему такая временная сложность**

*   **Основные переменные:** Пусть `E` — количество ребер в исходном графе (в нашем случае `E = 10`). Пусть `V` — количество вершин (`V = 10`).
*   **Цикл `while`:** Цикл `while remaining_edges:` (строка 11) может выполниться не более `E` раз, так как на каждой итерации мы удаляем хотя бы одно ребро (`pop` на строке 13).
*   **Операции внутри цикла:**
    ...
    *   **Самая "дорогая" операция:** `remaining_edges = {(a, b) for a, b in remaining_edges if ...}` (строка 20). Этот генератор множества проходит по *всем* ребрам, оставшимся в *текущем* множестве `remaining_edges`, и проверяет условие `a != u and a != v and b != u and b != v` для каждого ребра. В худшем случае (например, в начале выполнения, когда `remaining_edges` содержит все `E` ребер), этот генератор выполнит `E` проверок. В последующих итерациях количество ребер в `remaining_edges` уменьшается, но в худшем случае каждая итерация может занять до O(E) времени.
*   **Общая сложность:** Поскольку цикл может выполниться до `E` раз, и *внутри* каждой итерации может выполняться операция, занимающая O(E), общая временная сложность составляет O(E) * O(E) = **O(E²)**.
